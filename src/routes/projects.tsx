...
const STATUS_BADGE: Record<string, string> = {
	Backlog: "badge-outline border-base-content/20 text-base-content/40",
	Ready: "badge-outline border-base-content/30 text-base-content/50",
	"In progress": "badge-outline border-info text-info",
	"In review": "badge-outline border-warning text-warning",
	Done: "badge-outline border-success text-success",
};
...
function MilestoneGroup({
	...
	const total = milestoneInfo
		...
		: 0;
	const progress = total > 0 ? (milestoneInfo!.closedIssues / total) * 100 : 0;
...
function cardHashId(item: ProjectItem): string {
...
function ProjectItemCard({ item }: { item: ProjectItem }) {
	const html = item.body
		...
		: "";
	const repo = repoNameFromUrl(item.url);
	const hashId = cardHashId(item);
	const checkboxRef = useRef<HTMLInputElement>(null);
	const cardRef = useRef<HTMLDivElement>(null);

	useEffect(() => {
		const hash = decodeURIComponent(window.location.hash.slice(1));
		if (hash === hashId && checkboxRef.current) {
	...
	return (
		...
		>
			<input type="checkbox" ref={checkboxRef} onChange={handleToggle} />
			<div className="collapse-title grid gap-2 pr-10 sm:flex sm:items-center sm:gap-2 sm:pr-12">
				<span className="font-medium leading-snug sm:order-4 sm:min-w-0">
					{item.title}
				</span>
				<div className="flex flex-wrap items-center gap-2 sm:contents">
					{item.priority && (
						<span
							className={`badge badge-sm sm:order-1 ${PRIORITY_BADGE[item.priority]}`}
						>
							{item.priority}
						</span>
					)}
					{item.status && (
						<span
							className={`badge badge-sm sm:order-2 ${STATUS_BADGE[item.status] ?? "badge-outline border-base-content/30 text-base-content/50"}`}
						>
							{item.status}
						</span>
					)}
					<span className="text-base-content/35 text-sm sm:order-3">
						#{item.number}
					</span>
					{repo && (
						<span className="badge badge-sm badge-ghost text-base-content/40 text-xs sm:order-5 sm:ml-auto">
							{repo}
						</span>
					)}
				</div>
